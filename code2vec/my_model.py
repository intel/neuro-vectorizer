...
#    def _create_keras_model(self):
    ...
    def forward(self, input_dict, state, seq_lens):
        #print('hihihihhihihihi',input_dict)
        #print(state)
    ...
    def train(self):
        ...
        print(self.vocabs.path_vocab)
        print(self.vocabs.token_vocab)
        print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
        print(self.vocabs.path_vocab.size)
        print(self.vocabs.token_vocab.size)

        training_history = self.base_model.fit(
    ...
    def _save_inner_model(self, path):
        if self.config.RELEASE:
            self.base_model.save_weights(self.config.get_model_weights_path(path))
        else:
    ...
    def _create_inner_model(self):
        ...
        self.base_model.summary(print_fn=self.log)
    ...
    def _load_inner_model(self):
        ...
        if use_full_model:
            ...
            self.base_model.load_weights(self.config.model_weights_load_path)

        self.base_model.summary(print_fn=self.log)

    def _get_checkpoint(self):
        assert self.base_model is not None and self.base_model.optimizer is not None
        if self._checkpoint is None:
            ...
            self._checkpoint = tf.train.Checkpoint(
                ...
                optimizer=self.base_model.optimizer, model=self.base_model)
    ...
    def _get_vocab_embedding_as_np_array(self, vocab_type: VocabType) -> np.ndarray:
        ...
        weight = np.array(self.base_model.get_layer(embedding_layer_name).get_weights()[0])
